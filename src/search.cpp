...
namespace {
  ...
  int8_t Reductions[3][64][64]; // [pv][depth][moveNumber]

  template <bool PvNode> inline Depth reduction(Depth d, int mn, bool isCut) {

    return (Depth) Reductions[PvNode ? 0 : 1 + isCut][std::min(int(d) / ONE_PLY, 63)][std::min(mn, 63)];
  }
...
void Search::init() {
  ...
  for (hd = 1; hd < 64; hd++) for (mc = 1; mc < 64; mc++)
  {
      ...
      double allRed = 0.33 + log(double(hd)) * log(double(mc)) / 2.25;
      double cutRed = 0.33 + log(double(hd)) * log(double(mc)) / 2.25;
      Reductions[0][hd][mc] = (int8_t) ( pvRed >= 1.0 ? floor( pvRed * int(ONE_PLY)) : 0);
      Reductions[1][hd][mc] = (int8_t) (allRed >= 1.0 ? floor(allRed * int(ONE_PLY)) : 0);
      Reductions[2][hd][mc] = (int8_t) (cutRed >= 1.0 ? floor(cutRed * int(ONE_PLY)) : 0);
  }
...
split_point_start: // At split points actual search starts from here
    ...
    while ((move = mp.next_move<SpNode>()) != MOVE_NONE)
    {
          ...
          &&  bestValue > VALUE_MATED_IN_MAX_PLY)
      {
          ...
          Depth predictedDepth = newDepth - reduction<PvNode>(depth, moveCount, isCut);
      ...
      if (    depth > 3 * ONE_PLY
          ...
          &&  move != ss->killers[1])
      {
          ss->reduction = reduction<PvNode>(depth, moveCount, isCut);
