...
namespace {
  ...
  // Function prototypes
  template<bool Trace>
  Value do_evaluate(const Position& pos, Value& margin, Eval::Info& ei);

  template<Color Us>
  void init_eval_info(const Position& pos, Eval::Info& ei);

  template<Color Us, bool Trace>
  Score evaluate_pieces_of_color(const Position& pos, Eval::Info& ei, Score& mobility);

  template<Color Us, bool Trace>
  Score evaluate_king(const Position& pos, Eval::Info& ei, Value margins[]);

  template<Color Us>
  Score evaluate_threats(const Position& pos, Eval::Info& ei);

  template<Color Us>
  int evaluate_space(const Position& pos, Eval::Info& ei);

  template<Color Us>
  Score evaluate_passed_pawns(const Position& pos, Eval::Info& ei);

  Score evaluate_unstoppable_pawns(const Position& pos, Eval::Info& ei);
...
Value do_evaluate(const Position& pos, Value& margin, Eval::Info& ei) {
  ...
  Score evaluate_outposts(const Position& pos, Eval::Info& ei, Square s) {
  ...
  Score evaluate_pieces(const Position& pos, Eval::Info& ei, Score& mobility, Bitboard mobilityArea) {
