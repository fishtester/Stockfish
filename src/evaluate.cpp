...
Value do_evaluate(const Position& pos, Value& margin) {
  ...
  void init_eval_info(const Position& pos, EvalInfo& ei) {
        ...
        && pos.non_pawn_material(Us) >= QueenValueMidgame + RookValueMidgame)
    {
        ...
        ei.kingAttackersCount[Them][Us] = b ? std::max(1, popcount<Max15>(b) / 2) : 0;
				ei.kingAttackersWeight[Them][Us] = b ? KingAttackWeights[PAWN] : 0;
				ei.kingAdjacentZoneAttacksCount[Them][Us] = 0;
  ...
  Score evaluate_king(const Position& pos, EvalInfo& ei, Value margins[]) {
    ...
    if (   ei.kingAttackersCount[Us][Them] > 2
				|| (ei.kingAttackersCount[Us][Them] > 1 && ei.kingAdjacentZoneAttacksCount[Us][Them]))
    {
        ...
        attackUnits =  std::min(25, (ei.kingAttackersCount[Us][Them] * ei.kingAttackersWeight[Us][Them]) / 2)
				...
				attackUnits = (attackUnits * 9) / 8;

				attackUnits -= 3 * (std::min(15, (ei.kingAttackersCount[Us][Us] * ei.kingAttackersWeight[Us][Us]) / 2)
										 + 2 * ei.kingAdjacentZoneAttacksCount[Us][Us]) / 4;

				attackUnits -= mg_value(ei.pi->king_safety<Us>(pos, ksq)) / 28;
